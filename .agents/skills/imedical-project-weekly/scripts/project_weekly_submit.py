...
async def main():
    ...
    kept = [w for w in weeks if w < current_monday]
    removed = [w for w in weeks if w >= current_monday]
    if removed:
        print(f"Excluded current/future weeks (not allowed): {', '.join(w.isoformat() for w in removed)}")
    weeks = kept
