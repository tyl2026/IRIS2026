...
1. 计算需要补交的周（每月周一）
2. **禁止提交本周及之后的周报**：自动过滤掉周一日期 ≥ 本周一的周，只保留已过去的周（脚本强制拦截）
3. 过滤日志中已提交的周 → 跳过
4. 登录协同网（提示用户扫码）
5. 逐周：导航到 `WeekReport/UIWeeklyReport.aspx?modefiytime={周一日期}`
6. 有"生成周报"按钮 → 点击生成 → 关闭成功弹框 → 等数据加载
7. 点"提交周报"(BtnSubmit) → 打开iframe审批弹窗
8. iframe内点"提交" → "不能修改"弹框点确定 → "提交完成"点确定
9. 记录到日志，下个周
